	...
	run: (client, message, args) => {
		...
		const mensajes = {
			description: {
				...
				usuario: `\`\`\`js\nUsuario - ${message.member.user.username} -\`\`\``,
				list: `\`\`\`js\n Lista de servidores: \`\`\``,
			...
			field: {
				...
				ejemplo: { name: "Ejemplo", value: `${gremio.prefix()}ip add mc.imprialKnights:5030.es Imperial-Server`, inline: true },
				lista: [
		...
		if (opcion === 'list' || opcion === 'lista' || !opcion) {
			...
			if (opcion === 'clear' || opcion === 'limpiar') {
				...
				} else if (opcion === 'remove' || opcion === 'eliminar') { 
			...
			} else MessageCommon.embedField(mensajes.description.error.argumentos, mensajes.field.ejemplo)
		} else MessageCommon.embedField(mensajes.description.error.permisos, mensajes.field.permisos)
	}
}
